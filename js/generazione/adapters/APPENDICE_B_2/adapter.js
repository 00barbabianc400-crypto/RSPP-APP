/**
 * Adapter APPENDICE_B2_MISURE — Appendice B.2 (DOCX).
 * Copertina + tabella sito (sede, descrizione sito, processo, data sopralluogo).
 */
(function () {
  'use strict';

  const CODICE = 'APPENDICE_B2_MISURE';
  const NOME =
    'Appendice B.2: Programma delle misure per miglioramento livelli di sicurezza';
  ...
  function buildData(azienda, _rilevamenti, wizardInput) {
    ...
    return {
      ...
      MODULO_NUMERO: formatModuloNumero(w.modulo_numero, '40'),
  ...
  async function generateDocx(templateArrayBuffer, data) {
    ...
    try {
      ...
      const msg = repair?.formatDocxtemplaterErrors
        ...
        : err.message;
      throw new Error('Errore rendering Appendice B.2: ' + msg);
    }
