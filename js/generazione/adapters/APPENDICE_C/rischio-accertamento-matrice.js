/**
 * Matrice rischio lavorativo → accertamenti sanitari (Appendice C).
 */
(function () {
  'use strict';
  ...
  function formatPeriodicitaLabels(periodicitaIds) {
    return mergePeriodicitaIds(
      Object.fromEntries(
        (Array.isArray(periodicitaIds) ? periodicitaIds : []).map((id) => [id, [id]])
      ),
      null
    )
      .map((id) => PERIODICITA_OPTS.find((p) => p.id === id)?.label || id)
      .join(', ');
  }
