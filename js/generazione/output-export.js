...
(function () {
  ...
  async function exportMonitoraggioXlsx(stats, meta) {

    const ExcelJS = window.ExcelJS;

    if (!ExcelJS) throw new Error('ExcelJS non caricato — ricarica la pagina Monitoraggio');

    if (!stats) throw new Error('Nessun dato da esportare');
    ...
    wb.creator = 'RSPP-APP Monitoraggio';

    wb.created = new Date();



    const wsSum = wb.addWorksheet('Riepilogo', { views: [{ state: 'frozen', ySplit: 1 }] });

    wsSum.columns = [

      { header: 'Indicatore', key: 'k', width: 36 },
    ...
    const rows = [
      ...
      ['Generato il', stats.generato_il ? new Date(stats.generato_il).toLocaleString('it-IT') : ''],

      ['Query ms', meta?.elapsedMs ?? ''],

      ['Aziende', c.aziende],

      ['Profili attivi', c.profili_attivi],

      ['Associazioni azienda-profilo', c.associazioni],

      ['Valutazioni totali', totVal],

      ['Valutazioni Presenti', presenti],

      ['% Presenti', totVal ? ((presenti / totVal) * 100).toFixed(1) + '%' : ''],

      ['Righe teoriche (assoc × rischi)', teorico],

      ['Copertura catalogo %', teorico ? ((totVal / teorico) * 100).toFixed(1) + '%' : ''],

      ['Delta righe', s.delta_righe],

      ['Rischi attivi catalogo', c.rischi_attivi],

      ['Testi DVR', c.testi_dvr],

      ['Rilevamenti ambientali', c.rilevamenti],

      ['Utenti attivi', c.utenti_attivi],

      ['Profilo fasi definite', c.profilo_fasi],

      ['Profili con fasi', c.profili_con_fasi],

      ['DB totale bytes', db.database_bytes],

      ['DB valutazioni_rischio bytes', db.valutazioni_bytes],

      ['Aziende senza profili', a.aziende_senza_profili],

      ['Profili senza associazioni', a.profili_senza_associazioni],

    ];

    rows.forEach(([k, v, n]) => wsSum.addRow({ k, v: v ?? '', n: n || '' }));



    const wsTop = wb.addWorksheet('Top aziende');

    wsTop.columns = [

      { header: 'Azienda', key: 'nome', width: 42 },

      { header: 'Valutazioni', key: 'n', width: 14 },

    ];

    wsTop.getRow(1).font = { bold: true };

    top.forEach((t) => wsTop.addRow({ nome: t.nome, n: t.n }));
    ...
    triggerDownload(blob, 'monitoraggio_rspp_' + stamp + '.xlsx');
